app.initSocket = function () {
    ...
    $form.submit(function(){
        app.socket.emit('client-to-server', $newMessage.val());
        $newMessage.val('');
        return false;
    });

    app.socket.on('server-to-client', addMessageFromUser);

    app.socket.on('server-to-client-announce', addMessageFromAnnouncer);

    app.socket.emit('client-to-server-switch-room', app.roomId);
